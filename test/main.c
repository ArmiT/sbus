...
{   
    ...
    i = 0;
    for(i; i < SBUS_DATA_MAX_LENGTH;i++) {
        TEST_ASSERT_EQUAL(state.data[i], 0x00);
    }
}

void test_sbus_receive_valid(void)
{
    sbus_state_t state;
    state.receive_handler = &on_receive;
    state.transmit_handler = &on_transmit;
    ...
    // start byte -----       \     |     |     |     |     msb   lsb
    //                  \     |     |     |     |     |      |     |
    char package[8] = {0xAA, 0x01, 0x03, 0xE1, 0xE2, 0xE3, 0x38, 0x4F }

    sbus_init(&state);

    i = 0;
    for (i;i<8;i++) {
        sbus_process(&state, data[i]);
    }
...
// {   
//     t_sbus_connection connection;
//     sbus_init(connection, );
// }
